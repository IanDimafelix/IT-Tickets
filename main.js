...
const highlightTaskDone = () => {
  const index = _app_data__WEBPACK_IMPORTED_MODULE_1__["appData"]
      .tasksDone.indexOf(_app_data__WEBPACK_IMPORTED_MODULE_1__["appData"].tasksDone[0]);

  // Highlight the task done.
  const gray = 'RGB(151, 191, 56)';
  const white = 'RGB(185, 216, 112)';
	const electriclime = 'RGB(206, 255, 0,)';
  const timeItTakesToAddHighlight = '.3s';
  const timeItTakesToRemoveHighlight = '1.5s';
...
const generateTableWithHeader = () => {
  ...
  if (!tasksTable) {
    ...
    tasksTable.innerHTML = `
      ...
      <tr id="task-headings">
          <th></th>
          <th id="task" class="heading-cell">Bugs</th>
      ...
      <tr class="task" data-index="${index}">
        ...
        <td class="priority-cell">
          <select class="priority" data-index="${index}">
            <option value="P0" ${p0Selected}>HIGH</option>
            <option value="P1" ${p1Selected}>MID</option>
            <option value="P2" ${p2Selected}>LOW</option>
          </select>
        </td>
...
const addEmptyStateToDone = () => {
  ...
  div.innerHTML =
      ...
      'Bugs Fixed<br>will appear here.</p>';
};
...
const addEmptyStateToPlanner = () => {
  ...
  div.innerHTML =
      ...
      'Bugs List is empty.<br>Add a Bug Report below.</p>';
};
